...
static void sha256_256_init(sha256_ctx *ctx);
static void sha256_update(sha256_ctx *ctx, const void *data, size_t len);
static void sha256_final(sha256_ctx *ctx, unsigned char *result);
...
static void sha256_final(sha256_ctx *ctx, unsigned char *result)
{
    ...
    memset(ctx->block + fill, 0, 56-fill);

    uint64_t highCount = 0, lowCount = be64((ctx->nbytes * 8));
    memcpy(&ctx->block[56],&lowCount,8);
    sha256_transform(ctx, ctx->block);
    for (i=0; i<8; i++) {
        ctx->chain[i] = be32(ctx->chain[i]);
    }
